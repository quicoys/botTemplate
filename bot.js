...
const initBot = () => {
  ...
  const prefix = '!';
  const masterPrefix = '$';
  const admin = 'quicoys' //you can replace "quicoys" with your controller username
        ...
        if (message.startsWith(prefix)) {
            ...
            if (args[0].toLowerCase().startsWith('mindreader')) {
                if (args.length < 2) {
                    ...
                    setTimeout(() => {
                        bot.chat(`${username}; was your number ${num}?`)
            ...
            if (args[0].toLowerCase().startsWith('test')) {
                if (args.length < 2) {
                    ...
                    console.log('')
                    console.log('Online players on: ' + botArgs['host'] + ': ' + Object.keys(bot.players).length)
                    setTimeout(() => {
